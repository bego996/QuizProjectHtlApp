...
This app requires the QuizAppSchool backend to be running:
...
http://192.168.1.XXX:8080/  (Physical Device)
```
...
| Method | Endpoint | Description | Auth Required |
|--------|----------|-------------|---------------|
| GET | `/api/users/me/quiz-attempts` | Get user's quiz history | Yes |
| POST | `/api/users/me/quiz/start` | Start new quiz | Yes |
| PUT | `/api/users/me/quiz/submit/{id}` | Submit quiz answer | Yes |

### Admin - LLM Endpoints

| Method | Endpoint | Description | Auth Required |
|--------|----------|-------------|---------------|
| POST | `/api/llm/quiz/generate` | Generate quiz with AI | Admin |
| POST | `/api/llm/quiz/add` | Save generated quiz | Admin |
...
Entities: `questions`, `topics`, `answers`, `difficulties`, `statuses`, `userroles`, `userquestions`
...
| `network_security_config.xml` | Network security policy (HTTP allowlist) |
| `build.gradle.kts` | Dependencies and build configuration |
| `.claudeignore` | Files excluded from Claude Code context |

### Environment Configuration

Create `local.properties` for API keys (if needed):
```properties
...
- **Theme**: Material3 with dynamic theming support
- **Colors**: Primary, Secondary, Tertiary with variants
- **Typography**: Material3 default type scale
- **Components**: Cards, Buttons, TextFields, Lists, Dialogs

### Screens Overview

**User Screens (9+)**:
- Cover (Splash), Welcome, Login, Register
- Home, Categories, Topics, Subtopics
- Quiz, Quiz Results, Quiz Review
- Profile, Edit Profile, Settings

**Admin Screens (12+)**:
- Users Management, Active Quizzes, Generate Quizzes (AI)
- 8 Entity CRUD screens (User, Topic, Question, Answer, Difficulty, Status, UserRole, UserQuestion)
...
- **Comments**: Add compact comments to all functions
- **Naming**:
  - Screens: `*Screen.kt` (e.g., `LoginScreen`)
  - ViewModels: `*ViewModel.kt` with `@HiltViewModel`
  - Components: No suffix (e.g., `UserCard`)
- **State Management**: Use `StateFlow` with `*UiState` data classes
- **Async**: Use `viewModelScope.launch` for coroutines
- **Error Handling**: Always use `Result<T>` sealed class

### Code Review Checklist

- [ ] Code follows Kotlin conventions
- [ ] All new functions have comments
- [ ] ViewModels use StateFlow pattern
- [ ] Error handling implemented with Result type
- [ ] UI states handle loading, error, and success
...
- **Development Team** - HTL Students
- **Project Type** - DA-Project (Diploma Thesis)

---

## 🙏 Acknowledgments

- **Spring Boot Backend Team** - For the robust REST API
- **Ollama** - For local LLM integration
- **Anthropic Claude** - For development assistance via Claude Code
- **Android Team** - For Jetpack Compose and modern Android tools
- **HTL** - For project guidance and support
...
- **Issues**: [GitHub Issues](https://github.com/your-username/QuizAppHtl/issues)
- **Documentation**: See `CLAUDE.md` for detailed project documentation
- **Backend Setup**: Refer to backend repository documentation
...
- [ ] Dark mode theme
- [ ] Quiz timer and time-based scoring
...
- [x] JWT authentication with encrypted storage

### Completed
- [x] User authentication and profile management
- [x] Quiz taking and scoring system
- [x] Category and topic browsing
- [x] Admin CRUD operations for all entities
- [x] Material3 UI with Jetpack Compose
- [x] Clean Architecture implementation

---

**Made with ❤️ by HTL Students | Powered by Kotlin & Jetpack Compose**
